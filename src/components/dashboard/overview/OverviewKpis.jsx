...
function KpiCard({ icon, label, trendClassName, trendText, value, formatValue, meta, cardClassName }) {
  ...
  return (
    <article className="shadow-sm flex min-h-[150px] flex-col justify-center gap-2 rounded-2xl border border-[color:var(--app-border)] bg-[color:var(--app-card)] p-4 transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_14px_28px_rgba(25,37,86,0.12)] max-[1040px]:min-h-[160px]">
      <div className="mb-1.5 grid grid-cols-[auto_1fr_auto] items-center gap-2">
        ...
        <p className="m-0 text-[0.93rem] font-semibold text-[color:var(--app-text)]">{label}</p>
        <span className={`rounded-full px-2 py-1 text-[0.71rem] font-bold ${trendTone}`}>{trendText}</span>
      </div>
      <p className="m-0 text-[1.95rem] leading-[1.05] font-bold tracking-tight text-[color:var(--app-text)] max-[1040px]:text-[1.55rem]">
         <AnimatedNumber value={value} formatValue={formatValue} />
      </p>
      <p className="m-0 text-[0.79rem] text-[color:var(--app-muted)]">{meta}</p>
    </article>
  )
}
